...
export const SelectOption2: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.selectOptions(select, "pick2");
    await expect(canvas.getByText("Selected option: pick2")).toBeInTheDocument();
  },
};
...
export const SelectOption3: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const select = canvas.getByRole("combobox");
    await userEvent.selectOptions(select, "pick3");
    await expect(canvas.getByText("Selected option: pick3")).toBeInTheDocument();
  },
};
